...
ax1.set_xlabel(L"k^\mathrm{diag.}_{1 / \tau }", fontsize=fsize, labelpad=-1)
ax1.set_ylabel(L"k^\mathrm{pred.}_{1 / \tau }", fontsize=fsize)

ax1.legend(loc="lower right", bbox_to_anchor=(1.4, 0.0), fontsize=legsize, edgecolor="black", fancybox=false, framealpha=1, shadow=true)
...
ax2.set_xlabel(L"k^\mathrm{diag.}_{\mathrm{arrest}}", fontsize=fsize, labelpad=-1)
ax2.set_ylabel(L"k^\mathrm{diag.}_{\mathrm{EKE \ max.}}", fontsize=fsize)

ax2.legend(loc="lower right", bbox_to_anchor=(1.4, 0.0), fontsize=legsize, edgecolor="black", fancybox=false, framealpha=1, shadow=true)
...
ax3.plot(kxp, EKE_raw_mu0p25_hm1p25[:,slice_ind], color=h_color(h0s[2],hc_range,colors), linestyle="dashed", label=L"-1.25")
ax3.plot(kxp, EKE_raw_mu0p25_hm1p0[:,slice_ind], color=h_color(h0s[3],hc_range,colors), linestyle="dashed", label=L"-1.0")
ax3.plot(kxp, EKE_raw_mu0p25_hm0p75[:,slice_ind], color=h_color(h0s[4],hc_range,colors), linestyle="dashed", label=L"-0.75")
ax3.plot(kxp, EKE_raw_mu0p25_hm0p5[:,slice_ind], color=h_color(h0s[5],hc_range,colors), linestyle="dashed", label=L"-0.5")
ax3.plot(kxp, EKE_raw_mu0p25_hm0p25[:,slice_ind], color=h_color(h0s[6],hc_range,colors), linestyle="dashed", label=L"-0.25")


ax3.plot(kxp, EKE_raw_mu0p25_h1p25[:,slice_ind], color=h_color(h0s[12],hc_range,colors), label=L"1.25")
ax3.plot(kxp, EKE_raw_mu0p25_h1p0[:,slice_ind], color=h_color(h0s[11],hc_range,colors), label=L"1.0")
ax3.plot(kxp, EKE_raw_mu0p25_h0p75[:,slice_ind], color=h_color(h0s[10],hc_range,colors), label=L"0.75")
ax3.plot(kxp, EKE_raw_mu0p25_h0p5[:,slice_ind], color=h_color(h0s[9],hc_range,colors), label=L"0.5")
ax3.plot(kxp, EKE_raw_mu0p25_h0p25[:,slice_ind], color=h_color(h0s[8],hc_range,colors), label=L"0.25")

ax3t = ax3.twinx()
cc = 5.0e-9
ax3t.plot(kxp, cc .* kxp.^(-4), "k--", label=L"k_{x}^{-4}")
...
ax3.set_ylabel(L"\mathrm{EKE} \ \mathrm{[m}^2 \mathrm{s}^{-2} \mathrm{]}", fontsize=fsize)

ax3.tick_params(axis="both", labelsize=lsize)

ax3.legend(fontsize=legsize, ncols=2, bbox_to_anchor=(1.3, 1.0), loc="upper right", edgecolor="black", fancybox=false, framealpha=1, shadow=true)

ax3.grid()
ax3.set_axisbelow(true)


ax3t.set_xlim(1e-2, 5e0)
ax3t.set_yscale("log")
ax3t.set_xscale("log")

ax3t.set_ylim(1e-8, 1e-3)
ax3t.legend(fontsize=legsize, loc="lower right", bbox_to_anchor=(1.1, 0.2), edgecolor="black", fancybox=false, framealpha=1, shadow=true)
