...
function EKE_timescale(i, k, kxp, kyp, data_dir; yr_last=89.0, nus=zeros(13))
    ...
    fft_EKE = sqrt.(fftshift(EKE_2D)) ./ (2 * pi)   # 2pi removes factors from sqrt(grid.Ksq)
    ...
    for (i, kx) in enumerate(kxp ./ (2*pi*Ld))  # this correctly removes Ld and radian factors
        for (j, ky) in enumerate(kyp ./ (2*pi*Ld))
            ...
            # factor of 2pi req'd because grid.Ksq has factor of 2pi^2 and we have taken sqrt already
            tau_EKE[i,j] = fft_EKE[i, j] / l / ((2*pi))    # this is now in Hz, no normalization needed for radians, Ld, etc.
        end
    end
...
ax1.scatter(x1_all, y1_all, color="red", marker="P", s=msize, label=L"\kappa^* = 0.125 \mathrm{; \ } R^2=" * string(round(R2_mu0p125, digits=2)))
ax1.scatter(x2_all, y2_all, color="green", marker="X", s=msize, label=L"\kappa^* = 0.25 \mathrm{; \ \ } R^2=" * string(round(R2_mu0p25, digits=2)))
ax1.scatter(x3_all, y3_all, color="blue", marker="o", s=msize, label=L"\kappa^* = 0.5 \mathrm{; \ \ \ } R^2=" * string(round(R2_mu0p5, digits=2)))
...
ax1.legend(loc="lower right", fontsize=lsize)
...
ax2.scatter(x1_all, y1_all, color="red", marker="P", s=msize, label=L"\kappa^* = 0.125 \mathrm{; \ } R^2=" * string(round(R2_mu0p125, digits=2)))
ax2.scatter(x2_all, y2_all, color="green", marker="X", s=msize, label=L"\kappa^* = 0.25 \mathrm{; \ \ } R^2=" * string(round(R2_mu0p25, digits=2)))
ax2.scatter(x3_all, y3_all, color="blue", marker="o", s=msize, label=L"\kappa^* = 0.5 \mathrm{; \ \ \ } R^2=" * string(round(R2_mu0p5, digits=2)))
...
ax2.legend(loc="lower right", fontsize=lsize)
...
ax3.plot(kxp, EKE_mu0p25_hm1p25[:,slice_ind], color=h_color(h0s[2],hc_range,colors), linestyle="dashed")
ax3.plot(kxp, EKE_mu0p25_hm1p0[:,slice_ind], color=h_color(h0s[3],hc_range,colors), linestyle="dashed")
ax3.plot(kxp, EKE_mu0p25_hm0p75[:,slice_ind], color=h_color(h0s[4],hc_range,colors), linestyle="dashed")
ax3.plot(kxp, EKE_mu0p25_hm0p5[:,slice_ind], color=h_color(h0s[5],hc_range,colors), linestyle="dashed")
ax3.plot(kxp, EKE_mu0p25_hm0p25[:,slice_ind], color=h_color(h0s[6],hc_range,colors), linestyle="dashed")


ax3.plot(kxp, EKE_mu0p25_h1p25[:,slice_ind], color=h_color(h0s[12],hc_range,colors))
ax3.plot(kxp, EKE_mu0p25_h1p0[:,slice_ind], color=h_color(h0s[11],hc_range,colors))
ax3.plot(kxp, EKE_mu0p25_h0p75[:,slice_ind], color=h_color(h0s[10],hc_range,colors))
ax3.plot(kxp, EKE_mu0p25_h0p5[:,slice_ind], color=h_color(h0s[9],hc_range,colors))
ax3.plot(kxp, EKE_mu0p25_h0p25[:,slice_ind], color=h_color(h0s[8],hc_range,colors))
...
# savefig("./JPO_rev1_figs/arrest_klin_kdiag.png",bbox_inches="tight")
# mkdir("./JPO_rev2_figs")
