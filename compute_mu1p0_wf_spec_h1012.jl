...
data_dir_psi_ot = "/scratch/cimes/ml1994/QG/julia/data/JPO_2LQG_on_slope_data/mu1p0_hov/"
...
function calc_wf(z_ind_vec, h_ind, mu_ind, data_dir, model_params)
    ...
    nt_total = Int(3 * (last_yr - first_yr) * nt_per_year)
...
wf_m1p0_hm0p75 = calc_wf(zonal_ind, 4, 4, data_dir_psi_ot, model_params);  # zonal slice index, h_ind, mu_ind

nt = size(wf_m1p0_hm0p75)[2]
dt = a["jld_data"]["t"][6] - a["jld_data"]["t"][5]

# c = fftfreq(nt, 1/dt) ./ (grid.kr ./ (2 * pi))
fr = collect(fftshift(fftfreq(nt, 1/dt))) * 3600 * 24 ;

# xf = collect(range(0, Lx, Nx))

# kxr_wf = collect((rfftfreq(Nx, (xf[2]-xf[1])^-1)));


wf_out_m1p0_hm0p75 = Dict("w_f_spectrum" => wf_m1p0_hm0p75, "dt" => dt, "fr"=> fr)

jldsave("./wf_m1p0_hm0p75_total_EKE.jld"; wf_out_m1p0_hm0p75)


wf_m1p0_hm1p25 = calc_wf(zonal_ind, 2, 4, data_dir_psi_ot, model_params);  # zonal slice index, h_ind, mu_ind

wf_out_m1p0_hm1p25 = Dict("w_f_spectrum" => wf_m1p0_hm1p25, "dt" => dt, "fr"=> fr)

jldsave("./wf_m1p0_hm1p25_total_EKE.jld"; wf_out_m1p0_hm1p25)
